...
public Notify(client, String:format[], any:...)
{
	...
	{
		if  (IsClientInGame(i) && !IsFakeClient(i) && (i != client) && ((strlen(notify)==0) || (GetUserFlagBits(i)&ReadFlagString(notify)!=0)))
		{
...
public Action:VoteEnd(Handle:timer)
{
	voteInProgress = false;
	postVoteDelay = true;
	CreateTimer(POST_VOTE_DELAY, VoteDelay);
}
...
public LogVote(client,String:format[], any:...)
{
	...
	if (GetConVarBool(sendToLog))
	{
		...
		if (!client)
		{
			...
			Format(steamid, sizeof(steamid), "None");
		}
		else
		{
...
public hasVoteAccess(client, String:voteName[32])
{
	...
	if (!strcmp(gmode, "survival", false))
		survival=true;
...
public Action:Callvote_Handler(client, args)
{
	...
	if (voteInProgress)
	{
		PrintToChat(client, "\x04[!] \x01You cannot start a vote until the current vote ends.");
	...
	{
	       	PrintToChat(client,"\x04[!] \x01Invalid vote type: %s",voteName);
	...
	if (isInVoteTimeout(client))
	{
		...
		PrintToChat(client, "\x04[!] \x01You must wait \x03%.1f \x01seconds between votes.", GetConVarFloat(voteTimeout));
		return Plugin_Handled;		
	}
...
public Action:Kick_Vote_Logic(client, args)
{
	...
	if ((target<=0) || (!IsClientInGame(target)))
	{
		...
		Notify(client, "\x04[!] \x01%s tried to start a Kick Vote against %s but that is not a valid target.", initiatorName,arg2);
		PrintToChat(client, "\x04[SM] \x01If you are trying to call a manual kick vote the format is: 'callvote kick <user id>'");
		return Plugin_Handled;
	}
...
//	if ((GetClientTeam(target) == TEAM_INFECTED) && IsPlayerAlive(target))
//	{
...
//			Notify(client,"\x04[!] \x01%s tried to start a Kick Vote against %s but tanks cannot be kicked.",initiatorName,targetName);
//			return Plugin_Handled;
//		}
//	}
	...
	if (GetClientTeam(client) == TEAM_SPECTATOR)
	{
		...
		Notify(client, "\x04[!] \x03%s \x01tried to start a Kick Vote against \x03%s \x01but spectators are not allowed to kick.",initiatorName,targetName);
		return Plugin_Handled;
	}
	...
	if (GetConVarBool(kickImmunity))
	{
		...
		if (isAdmin(targetAdminId)){
			...
			if (!CanAdminTarget(clientAdminId, targetAdminId))
			{
				...
				Notify(client, "\x04[Vote] \x03%s \x01tried to start a Kick Vote against \x03%s \x01but he has immunity.",initiatorName,targetName);
				return Plugin_Handled;
			}
		}
	}
...
public Action:Veto_Handler(client, args)
{
	...
	if (!voteInProgress || postVoteDelay) 
	{
		...
		if (client!=0)
		{
			PrintToChat(client, "\x04[SM] \x01No current vote to veto."); 
		}
	...
	if (client==0){
		...
		Notify(0, "\x04[SM] \x01The Server has vetoed this vote.");
		return Plugin_Continue;
	...
	{	
		...
		Notify(client, "\x04[SM] \x03%s \x01has vetoed this vote.",vetoerName);
		return Plugin_Continue;
	...
	Notify(client, "\x04[!] \x03%s \x01tried to veto a vote but does not have access.",vetoerName);
	return Plugin_Handled;
}
...
public Action:PassVote_Handler(client, args)
{
	if (!voteInProgress || postVoteDelay) 
	{
		...
		if (client)
		{
			PrintToChat(client, "\x04[SM] \x01No current vote to pass."); 
		}
		return Plugin_Handled;
	}
	...
	if (!client)
	{
		...
		//PrintToChatAll("\x04[SM] \x01The Server has passed this vote.");
		Notify(0, "\x04[SM] \x01The Server has passed this vote.");
		return Plugin_Continue;
	}
	...
	{	
		...
		LogVote(client,"has passed a vote.");
		//PrintToChatAll("\x04[SM] \x01%s has passed this vote.",passerName);
		Notify(client, "\x04[SM] \x03%s \x01has passed this vote.",passerName);
		return Plugin_Continue;
		
	}
	LogVote(client,"failed to veto vote. Reason: Access");
	//PrintToChatAll("\x04[SM] \x01%s tried to pass a vote but does not have access.",passerName);
	Notify(client, "\x04[!] \x03%s \x01tried to pass a vote but does not have access.",passerName);
	return Plugin_Handled;
}
...
public Action:CustomVote_Handler(client, args)
{
	...
	else
	{
		...
		Notify(client, "\x04[!] \x03%s \x01tried starting a Custom vote but one is already in progress.",initiatorName);
	}
...
public Action:Custom_Vote_Logic(client, args)
{
	...
	if (!customVoteInProgress)
	{
		...
		Notify(client, "\x04[!] \x03%s \x01is starting a Custom vote.",initiatorName);
