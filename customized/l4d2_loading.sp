	...
	if (IsCountDownStoppedOrRunning() && !GetConVarBool(scrimMode))
	{
		for (new i = 1; i <= MaxClients; i++)
		{
			if (IsClientConnected(i) && IsClientInGame(i) && !IsFakeClient(i) && GetClientTeam(i) == 2)
			{
				...
				{
					if (!IsSoleSurvivor() && !IsTeamVersus())
					{
					...
					else if (IsSoleSurvivor() && display == 0 && !IsTeamVersus())
					{
...
stock bool:IsTeamVersus()
{
	...
	return StrContains(Mode, "teamversus", false) > -1;
}
...
public Action:StartTimer(Handle:timer)
{
	...
	else if (countDown++ >= GetConVarInt(isFirstRound ? prepareTime1st : prepareTime2nd) - 1)
	{
		...
		if (!isFirstMap)
		{
			if (checkPointDoorEntityStart != 0) SpawnFakeDoor(checkPointDoorEntityStart, false, 0.1);
		}
		UnFreezePlayers();
...
GetCheckPointDoorStart()
{
	...
	if (foundAnOrigin)
	{
		...
		for (new i = 0; i < 4; i++)
		{
			if (checkPointDoorEntityIds[i] && (flags[i] == 8192 || flags[i] == 0))
			{
			...
			#if DEBUG_DOOR
			else if (checkPointDoorEntityIds[i]) LogError("[%s] Invalid entity id or invalid spawnflags [ ent = %i | spawnflags = %i ]", TAG, checkPointDoorEntityIds[i], flags[i]);
			#endif
		}
		...
		if (checkPointDoorEntityStart != 0)
		{
			...
			SpawnFakeDoor(checkPointDoorEntityStart, true, 5.0);
		}
...
SpawnFakeDoor(ent, bool:cantOpen, Float:Delay)
{
	new Handle:datapack = CreateDataPack();
	WritePackCell(datapack, ent);
	WritePackCell(datapack, cantOpen);
	CreateTimer(Delay, SpawnFakeDoorDelayed, datapack);
}

public Action:SpawnFakeDoorDelayed(Handle:timer, Handle:datapack)
{
	ResetPack(datapack);
